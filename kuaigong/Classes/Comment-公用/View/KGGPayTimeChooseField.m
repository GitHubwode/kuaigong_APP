...
@interface KGGPayTimeChooseField ()<UIPickerViewDelegate,UIPickerViewDataSource>
...
@implementation KGGPayTimeChooseField
...
- (void)ensureButtonAction{
    ...
    if ([self.timeDelegate respondsToSelector:@selector(payTimeChooseFieldEnsureButtonClick)]) {
        [self.timeDelegate payTimeChooseFieldEnsureButtonClick];
    }
...
- (NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component{
    return self.timeDatasource[row];
}


// 选中某一行的时候调用
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    _selectedText = self.timeDatasource[row];
}
