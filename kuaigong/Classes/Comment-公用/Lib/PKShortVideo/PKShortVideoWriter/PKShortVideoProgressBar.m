...
static NSInteger const PKProgressItemWidth = 4;
...
@property (nonatomic, strong) UIView *minProgressView;
...
- (instancetype)initWithFrame:(CGRect)frame themeColor:(UIColor *)themeColor duration:(NSTimeInterval)duration miniDuration:(NSTimeInterval)miniDuration {
    ...
    if (self) {
        ...
        _minProgressView = [[UIView alloc]initWithFrame:CGRectMake((miniDuration / duration) * frame.size.width, 0, 1.f, frame.size.height)];
        _minProgressView.backgroundColor = [UIColor redColor];
        [self addSubview:_minProgressView];
