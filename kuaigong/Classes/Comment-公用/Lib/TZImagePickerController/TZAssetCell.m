...
- (void)setMaxImagesCount:(NSInteger)maxImagesCount {
    ...
    if (!self.selectPhotoButton.hidden) {
        self.selectPhotoButton.hidden = maxImagesCount == 0;
    }
    if (!self.selectImageView.hidden) {
        self.selectImageView.hidden = maxImagesCount == 0;
    }
}
...
- (void)setType:(TZAssetCellType)type {
    ...
    }else if(TZAssetCellTypeVideo == type) {
...
- (UIImageView *)selectImageView {
    if (_selectImageView == nil) {
        ...
        selectImageView.frame = CGRectMake(self.tz_width - 24 - 2, 2, 24, 24);
...
- (UIImageView *)viewImgView {
    if (_viewImgView == nil) {
        UIImageView *viewImgView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"icon_video_small"]];
        viewImgView.tz_left = 5.f;
        viewImgView.tz_top = 2.f;
...
- (BOOL)canSelected{
    ...
    if (_type == TZAssetCellTypeVideo) {
        NSArray *times = [self.timeLength.text componentsSeparatedByString:@":"];
        NSInteger minute = [times.firstObject integerValue];
        return minute <= 10;
    }
