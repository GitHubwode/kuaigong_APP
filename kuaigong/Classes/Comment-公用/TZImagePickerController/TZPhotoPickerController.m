...
@interface TZPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate,PKRecordShortVideoDelegate> {
...
- (void)viewDidLoad {
    ...
    }else if (tzImagePickerVc.allowPickingVideo){
        _showTakePhotoBtn = ([[TZImageManager manager] isCameraRollAlbum:_model.name]);
    }
...
- (void)configCollectionView {
    ...
    CGFloat top = 44;
    if (iOS7Later) top += 20;
#pragma mark - XC-修改 tzImagePickerVc.maxImagesCount > 1
    CGFloat collectionViewHeight = tzImagePickerVc.maxImagesCount > 0 ? self.view.tz_height - 50 - top : self.view.tz_height - top;
...
- (void)configBottomToolBar {
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    if (tzImagePickerVc.maxImagesCount <= 0) return;
    ...
    [_previewButton setTitleColor:SNHTitleTextColor forState:UIControlStateNormal];
    ...
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        [_originalPhotoButton setTitleColor:SNHTitleTextColor forState:UIControlStateSelected];
        ...
        _originalPhotoLable.font = [UIFont systemFontOfSize:15];
        _originalPhotoLable.textColor = SNHTitleTextColor;
    ...
    _numberImageView.tz_centerY = _okButton.tz_centerY;
    _numberImageView.tz_left = self.view.tz_width - 56 - 20;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    KGGLog(@"%zd - %zd - %zd - %zd",tzImagePickerVc.sortAscendingByModificationDate,indexPath.row,_showTakePhotoBtn,_models.count);
    
    if (((tzImagePickerVc.sortAscendingByModificationDate && indexPath.row >= _models.count) || (!tzImagePickerVc.sortAscendingByModificationDate && indexPath.row == 0)) && _showTakePhotoBtn && tzImagePickerVc.allowPickingImage) {
    ...
    if (tzImagePickerVc.allowPickingVideo && ((tzImagePickerVc.sortAscendingByModificationDate && indexPath.row >= _models.count) || (!tzImagePickerVc.sortAscendingByModificationDate && indexPath.row == 0)) && _showTakePhotoBtn) {
        
        TZAssetCameraCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"TZAssetCameraCell" forIndexPath:indexPath];
        cell.imageView.image = [UIImage imageNamedFromMyBundle:tzImagePickerVc.takeVideoImageName];
        return cell;
    }
...
//        } else {
        ...
        videoPlayerVc.backButtonClickBlock = ^(){
            [weakSelf.collectionView reloadData];
            [weakSelf refreshBottomToolBarStatus];
        };
        videoPlayerVc.currentIndex = index;
...
#define push @#clang diagnostic pop
        ...
        video.delegate = self;
        KGGNavigationController *nav = [[KGGNavigationController alloc] initWithRootViewController:video];
