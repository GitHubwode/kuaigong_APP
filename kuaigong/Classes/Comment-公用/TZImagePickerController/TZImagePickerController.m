...
//    self.navigationBar.barStyle = UIBarStyleBlack;
    ...
    // 默认的外观，你可以在这个方法后重置
    self.oKButtonTitleColorNormal   = SNHGoldenThemeColor;
    self.oKButtonTitleColorDisabled = [UIColor colorWithRed:(196/255.0) green:(158/255.0) blue:(119/255.0) alpha:0.5];
//    [UIColor colorWithRed:(83/255.0) green:(179/255.0) blue:(17/255.0) alpha:0.5];
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    ...
    if (self) {
        ...
        self.barItemTextFont = KGGLightFont(15);
        self.barItemTextColor = SNHTitleTextColor;
...
- (void)configDefaultImageName {
    self.takePictureImageName = @"photo";
    self.takeVideoImageName = @"shooting";
    
    self.photoSelImageName = @"icon_circle_selector_full";
    self.photoDefImageName = @"icon_circle_selector_empty";
    
    self.photoNumberIconImageName = @"icon_chosedot";
    self.photoPreviewOriginDefImageName = @"preview_original_def.png";
    self.photoOriginDefImageName = @"photo_original_def.png";
    self.photoOriginSelImageName = @"icon_original_chosedot";
}
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated {
    ...
    if (self.childViewControllers.count > 0) { // 如果viewController不是最早push进来的子控制器
        ...
        [backButton setImage:[UIImage imageNamed:@"nav_icon_back_photo_white"] forState:UIControlStateNormal];
        [backButton setImage:[UIImage imageNamed:@"nav_icon_back_photo_white"] forState:UIControlStateHighlighted];
        [backButton setTitle:[NSString stringWithFormat:@" %@",[NSBundle tz_localizedStringForKey:@"Back"]] forState:UIControlStateNormal];
        backButton.titleLabel.font = KGGLightFont(15);
        [backButton setTitleColor:KGGTitleTextColor forState:UIControlStateNormal];
